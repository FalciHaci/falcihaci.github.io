	...
	//Update shop card
		...
		let updateCard = () => {
			...
			shopTable.find('.quantity input[type="number"]').on('change', function () {
				/// Fro HTML start
				let self = $(this);
				let priceValue = parseFloat(self.closest('.cart_item').find('.product-price .amount').text().slice(1));
				let currency = self.closest('.cart_item').find('.product-price .amount').text()[0];
				let inputValue = parseFloat(self.val()) < 1 ? 1 : parseFloat(self.val());
				self.closest('.cart_item').find('.product-subtotal .amount').html(`${priceValue*inputValue} <span>${currency}</span>`);
				let suandafiyat = priceValue*inputValue;
				const round = (x, n) =>
					Number(parseFloat(Math.round(x * Math.pow(10, n)) / Math.pow(10, n)).toFixed(n));

				$('.totalshop').find('.Subtotal .amount').html(`${suandafiyat} <span>${currency}</span>`);
				$('.totalshop').find('.Total .amount').html(`${suandafiyat} <span>${currency}</span>`);
